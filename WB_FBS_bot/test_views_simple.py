...
try:
    ...
    try:
        ...
        # Для теста запрашиваем все товары батчами (лимит API - 100 за раз)
        print(f"   📊 Всего товаров: {len(nm_ids)}")
        print(f"   ⚠ API лимит: 100 товаров за запрос, запросим несколько батчей для поиска просмотров")
    except Exception as e:
    ...
    # Запрашиваем батчами по 100 товаров, чтобы найти все просмотры
    all_views_stats = {}
    
    batch_size = 100
    total_batches = (len(nm_ids) + batch_size - 1) // batch_size
    
    # Для теста ограничиваем до 500 товаров (5 батчей)
    max_products_for_test = min(len(nm_ids), 500)
