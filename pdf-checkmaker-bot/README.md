...
### Готовые примеры для скриншотов / тестов
...
Их можно сразу загрузить в бота, чтобы быстро получить красивый PDF для скриншотов.

### Скриншоты для портфолио

Для единообразия с другими ботами скриншоты лучше складывать сюда:

- Папка: `screenshots/` в корне `pdf-checkmaker-bot`
- Рекомендуемые имена файлов:
  - `01_start_menu.png` — главное меню / экран после `/start`
  - `02_upload_files.png` — экран после загрузки CSV и HTML
  - `03_pdf_ready.png` — сообщение с готовым PDF‑чеком

Эти имена удобно использовать в README и при публикации в портфолио.
