...
Бот и Mini App разворачиваются в Docker: бот, API (Gunicorn/Flask) и Nginx в отдельных контейнерах. Запуск: `docker compose up -d --build`. Конфигурация в `docker-compose.yml`, подробная инструкция по серверу и портам — в **MINIAPP-NGINX.md**.
